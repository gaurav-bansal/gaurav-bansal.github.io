...
int main(void){
	int g_cnt = 0;
	int f_led = 0;
	char *logfile = "/home/pi/gaurav-bansal.github.io/pressurelog.txt";
	FILE *fp;
	...
	//while(1){
		...
		fp = fopen(logfile, "a"); 
		fprintf(fp, "%4d/%2d/%2d %2d:%2d:%2d\tPressure : %d (Pa)\tTemperature : %.1f (degC)\n", year, mon, day, hour, min, sec, pres, temp);
		fclose(fp);
		printf("%4d/%2d/%2d %2d:%2d:%2d\tPressure : %d (Pa)\tTemperature : %.1f (degC)\n", year, mon, day, hour, min, sec, pres, temp);
